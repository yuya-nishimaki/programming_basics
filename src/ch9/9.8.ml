...
let person4 = {name = "真一"; height = 1.86; weight = 83.; birthday = (8, 23); blood_type = "O"}
...
(* 目的：person_t 型のデータリスト lst を受け取って、乙女座の人の名前のみからなるリストを返す *)
(* otomeza : person_t list -> string list *)
let rec otomeza lst = match lst with
    [] -> []
    | {name = n; height = h; weight = w; birthday = (month, day); blood_type = bt} :: rest
